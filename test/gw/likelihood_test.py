...
class TestBasicGWTransient(unittest.TestCase):
    ...
    def test_log_likelihood(self):
        ...
        self.assertAlmostEqual(self.likelihood.log_likelihood(self.parameters), -4032.4397343470005, 3)
...
class TestHyperbolicGWTransient(unittest.TestCase):
    ...
    def _log_bessel_k(order, value):
        return np.log(kve(order, value)) - value

    @classmethod
    def _manual_log_density(cls, abs2_over_scale2, alpha, delta):
        radius = np.sqrt(delta ** 2 + abs2_over_scale2)
        argument = alpha * radius
        return (
            np.log(alpha)
            - np.log(2.0 * np.pi * delta)
            - cls._log_bessel_k(1, alpha * delta)
            + cls._log_bessel_k(0, argument)
        )
    ...
    def test_log_likelihood_matches_direct_calculation(self):
        ...
        for ifo in self.interferometers:
            ...
            r = ifo.frequency_domain_strain[mask] - h_f[mask]
            scale2 = ifo.power_spectral_density_array[mask] * self.duration / 4.0
            abs2_over_scale2 = (r.real ** 2 + r.imag ** 2) / scale2
            manual += np.sum(
                self._manual_log_density(
                    abs2_over_scale2=abs2_over_scale2,
                    alpha=alpha,
                    delta=delta,
                )
                - np.log(scale2)
    ...
    def test_multiband_log_likelihood_matches_direct_calculation(self):
        ...
        for ifo in self.interferometers:
            ...
            for band_index, (alpha, delta) in enumerate(zip(band_alphas, band_deltas)):
                ...
                manual += np.sum(
                    self._manual_log_density(
                        abs2_over_scale2=abs2_over_scale2[band_mask],
                        alpha=alpha,
                        delta=delta,
                    )
