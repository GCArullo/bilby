...
class TestROQLikelihood(ROQBasisMixin, unittest.TestCase):
    ...
    def test_matches_non_roq(self):
        ...
        non_roq_llr = self.non_roq.log_likelihood_ratio(self.test_parameters)
        self.assertLess(
...
class TestROQLikelihoodHDF5(unittest.TestCase, ROQBasisMixin):
    ...
    def test_method_switches_basis(self, method):
        basis_linear = self._path_to_basis
        basis_quadratic = self._path_to_basis
        interferometers = bilby.gw.detector.InterferometerList(["H1", "L1"])
        interferometers.set_strain_data_from_power_spectral_densities(
            sampling_frequency=self.sampling_frequency, duration=self.duration, start_time=1.2 - self.duration + 1
        )
        for ifo in interferometers:
