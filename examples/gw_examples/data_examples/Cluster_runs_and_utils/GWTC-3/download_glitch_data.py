...
"""Download the public de-glitched frames required by eight GWTC-3 templates."""
...
from prepare_gwtc3 import common
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--event",
        action="append",
        help="Download only frames containing these GWTC-3 event IDs",
    )
...
def main() -> int:
    ...
    if args.event:
        event_times = [common.event_gps(event) for event in args.event]
        files = [
            item
            for item in files
            if any(
