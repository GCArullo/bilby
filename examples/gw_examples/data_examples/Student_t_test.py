...
nu_min, nu_max = (
    2.1,
    200,
)  # Range for log-uniform prior on nu (if infer_nu=True). Must be > 2 for finite variance in 2D.
alpha_min, alpha_max = 1e-6, 30  # HyperWave-style uniform prior on alpha.
delta_min, delta_max = 1e-6, 30  # HyperWave-style uniform prior on delta.
num_frequency_bands = (
    2  # For N > 1, sample nu_i or alpha_i for each band.
)
...
if single_par:
    ...
    priors = bilby.gw.prior.BBHPriorDict(filename=f"Priors/{event}.prior")
...
if likelihood_type == "Gaussian":
    ...
    if num_frequency_bands == 1:
        ...
        for band_index in range(1, num_frequency_bands + 1):
            priors[f"nu_{band_index}"] = bilby.core.prior.LogUniform(
                nu_min, nu_max, name=f"nu_{band_index}"
            )
